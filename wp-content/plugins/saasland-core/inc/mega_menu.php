...
class Saasland_Mega_Menu{
    ...
    public function custom_walker_nav_menu_start_el( $item_output, $item, $depth, $args ){
        ...
        if ($item->mega_menu!=0) {
            $item_output .= sprintf('<a href="%s" class="nav-link dropdown-toggle" aria-expanded="false" role="button" aria-haspopup="true" data-toggle="dropdown">%s</a>', $url, $item->title);
        }else {
            $is_carrot = ( $has_children && 1 === $depth || $has_children && 0 === $depth) ? ' <span class="arrow_carrot-right"></span> ' : '';
            $item_output .= sprintf('<a href="%s" class="nav-link">%s %s</a>', $url, $item->title, $is_carrot);
        }
