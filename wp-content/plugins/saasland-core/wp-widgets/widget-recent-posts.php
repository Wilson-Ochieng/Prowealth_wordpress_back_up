		...
		foreach ( $r->posts as $recent_post ) : ?>
            ...
            <div <?php post_class('media post_item') ?>>
                    ...
                    <?php if(has_post_thumbnail($recent_post->ID)) { ?>
                        <?php echo get_the_post_thumbnail($recent_post->ID, 'saasland_85x70', array('class' => 'media-object')) ?>
                    <?php } ?>

                <div class="media-body">
                    <a href="<?php the_permalink( $recent_post->ID ); ?>" title="<?php echo esc_html($title); ?>">
                        <h3> <?php saasland_limit_latter($title, 24, ''); ?> </h3>
                    </a>
                    <?php
                    if ( $show_date ) : ?>
                        <a class="entry_post_info" href="<?php saasland_day_link() ?>"> <?php echo get_the_date( get_option('date_format'), $recent_post->ID ); ?> </a>
                    <?php endif; ?>
                </div>
            </div>
        <?php endforeach; ?>
