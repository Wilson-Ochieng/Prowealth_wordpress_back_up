...
if (substr(gordon_get_theme_option('blog_style'), 0, 7) == 'masonry') {
...
if (have_posts()) {
	...
	$gordon_classes = 'posts_container '
						. (substr(gordon_get_theme_option('blog_style'), 0, 7) == 'classic' ? 'columns_wrap' : 'masonry_wrap');
	...
	if (!$gordon_sticky_out) {
		if (gordon_get_theme_option('first_post_large') && !is_paged() && !in_array(gordon_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
			get_template_part( 'content', 'excerpt' );
		}
		
		?><div class="<?php echo esc_attr($gordon_classes); ?>"><?php
	}
	while ( have_posts() ) { the_post(); 
		if ($gordon_sticky_out && !is_sticky()) {
			$gordon_sticky_out = false;
			?></div><div class="<?php echo esc_attr($gordon_classes); ?>"><?php
		}
		get_template_part( 'content', $gordon_sticky_out && is_sticky() ? 'sticky' : 'classic' );
	}
