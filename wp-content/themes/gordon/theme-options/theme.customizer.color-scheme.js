/* global gordon_color_schemes, gordon_dependencies, Color */
...
( function( api ) {
	...
	for (var i in gordon_color_schemes) {
		cssTemplate[i] = wp.template( 'gordon-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'gordon-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+gordon_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(gordon_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', gordon_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', gordon_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			gordon_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		gordon_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		gordon_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		gordon_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function gordon_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			var id = jQuery(this).attr('id');
			if (id == undefined) return;
			id = id.replace('customize-control-', '');
			var depend = false;
			for (var fld in gordon_dependencies) {
				if (fld == id) {
					depend = gordon_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('gordon_options_no_use');
				} else {
					jQuery(this).hide().addClass('gordon_options_no_use');
				}
			}
		});
	}
	...
	function gordon_customizer_refresh_preview(obj) {
		...
		if (id == 'color_scheme_editor') {
			gordon_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in gordon_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					gordon_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							gordon_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							gordon_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in gordon_theme_fonts) {
					for (rule in gordon_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							gordon_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) gordon_customizer_update_css();
		}
	...
	// Actions buttons
	function gordon_customizer_actions(e) {
	...
	// Store new value in the color table
	function gordon_customizer_update_color_scheme(opt, value) {
		gordon_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(gordon_serialize(gordon_color_schemes))
	}
	
	// Store new value in the theme fonts
	function gordon_customizer_update_theme_fonts(opt, rule, value) {
		gordon_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function gordon_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=gordon_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in gordon_theme_fonts) {
	...
	function gordon_customizer_change_color_scheme(value) {
		updateCSS = false;
		for (var opt in gordon_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( gordon_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', gordon_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', gordon_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		gordon_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function gordon_customizer_update_css() {
		...
		var fonts = gordon_customizer_add_theme_fonts(gordon_theme_fonts);

		// Make styles and add into css
		css += gordon_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in gordon_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in gordon_color_schemes[scheme].colors) {
				colors[i] = gordon_color_schemes[scheme].colors[i];
			}
	...
	function gordon_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + gordon_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + gordon_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + gordon_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function gordon_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function gordon_customizer_prepare_html_value(val) {
