...
if ( !function_exists('gordon_need_page_title') ) {
	function gordon_need_page_title() {
		return !is_front_page() && apply_filters('gordon_filter_need_page_title', true);
	}
}
...
if ( !function_exists('gordon_show_layout') ) {
	function gordon_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('gordon_get_logo_image') ) {
	function gordon_get_logo_image($type='') {
		$logo_image = '';
		if (gordon_get_retina_multiplier(2) > 1)
			$logo_image = gordon_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = gordon_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('gordon_get_header_video') ) {
	function gordon_get_header_video() {
		$video = '';
		if (apply_filters('gordon_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (gordon_check_theme_option('header_video')) {
				$video = gordon_get_theme_option('header_video');
...
if ( !function_exists('gordon_show_post_meta') ) {
	function gordon_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('gordon_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php gordon_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('gordon_filter_get_post_date', gordon_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			gordon_show_layout(gordon_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				gordon_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'gordon'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'gordon' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('gordon_show_post_featured') ) {
	function gordon_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => gordon_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => gordon_is_on(gordon_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : gordon_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = gordon_get_post_image();
				if (!empty($image)) 
					$image = gordon_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = gordon_get_no_image('', !empty($args['show_no_image']));
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . gordon_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$gordon_attr = gordon_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($gordon_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($gordon_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						gordon_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = gordon_get_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						gordon_show_layout($output);
					...
					if (!empty($image)) {
						...
						gordon_hovers_add_icons($args['hover'], array('image' => $image));
					}
				} else if (!empty($args['show_no_image']) && ($image=gordon_get_no_image())!='') {
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					//NEW
					$post_content = gordon_get_post_content();
					$video = gordon_get_post_video($post_content, false);
					if (empty($video))
						$video = gordon_get_post_iframe('', false);
					if ( empty( $video ) ) {
						// Only get video from the content if a playlist isn't present.
						$post_content_parsed = gordon_filter_post_content( $post_content );
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = gordon_get_post_audio('', false);
					if (empty($audio))
						$audio = gordon_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = gordon_get_post_audio($audio);
						...
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = gordon_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = gordon_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !gordon_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php gordon_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !gordon_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php gordon_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							gordon_show_layout($audio); 
						?></div><?php
					}
				}
				
				// Put optional info block over the thumb
				gordon_show_layout($post_info);
				?></div><?php
			}
		}
	}
...
if ( !function_exists('gordon_get_no_image') ) {
	function gordon_get_no_image($no_image='', $need=false) {
		static $img = '';
		if (empty($img)) {
			$img = gordon_get_theme_option( 'no_image' );
			// Remove false in next line if you want display theme-specific image in the posts without featured image
			if ($need && empty($img)) $img = gordon_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('gordon_add_bg_in_post_nav') ) {
	function gordon_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = gordon_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), gordon_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), gordon_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'gordon-main', $css );
	}
}

// Show related posts
if ( !function_exists('gordon_show_related_posts') ) {
	function gordon_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h4 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('Related Articles', 'gordon');
				?></h4>
...
if ( !function_exists('gordon_show_portfolio_posts') ) {
	function gordon_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? gordon_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = gordon_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = gordon_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the gordon_ajax_get_posts action
if ( !function_exists( 'gordon_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_gordon_ajax_get_posts',			'gordon_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_gordon_ajax_get_posts',	'gordon_ajax_get_posts_callback');
	function gordon_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			gordon_storage_set('blog_mode', 'blog');
			gordon_storage_set('options_meta', get_post_meta($id, 'gordon_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => gordon_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'gordon');
		}
...
// Show pagination
if ( !function_exists('gordon_show_pagination') ) {
	function gordon_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = gordon_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'gordon' ),
				'next_text' => esc_html__( '>', 'gordon' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'gordon' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'gordon'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'gordon') : esc_html__('Newest posts', 'gordon') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'gordon') : esc_html__('Older posts', 'gordon'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
