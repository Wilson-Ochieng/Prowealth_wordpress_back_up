...
if (!function_exists('gordon_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'gordon_hovers_theme_setup3', 3 );
	function gordon_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		gordon_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'gordon'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'gordon') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'gordon'),
					'slide_left'	=> esc_html__('Slide from Left',	'gordon'),
					'slide_right'	=> esc_html__('Slide from Right',	'gordon'),
					'slide_top'		=> esc_html__('Slide from Top',		'gordon'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'gordon'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'gordon'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'gordon') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'gordon'),
					'icon'	=> esc_html__('Icon',	'gordon'),
					'icons'	=> esc_html__('Icons',	'gordon'),
					'zoom'	=> esc_html__('Zoom',	'gordon'),
					'fade'	=> esc_html__('Fade',	'gordon'),
					'slide'	=> esc_html__('Slide',	'gordon'),
					'pull'	=> esc_html__('Pull',	'gordon'),
					'border'=> esc_html__('Border',	'gordon')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('gordon_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'gordon_hovers_theme_setup9', 9 );
	function gordon_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'gordon_hovers_frontend_scripts', 1010 );
		add_filter( 'gordon_filter_localize_script','gordon_hovers_localize_script' );
		add_filter( 'gordon_filter_merge_scripts',	'gordon_hovers_merge_scripts' );
		add_filter( 'gordon_filter_merge_styles',	'gordon_hovers_merge_styles' );
		add_filter( 'gordon_filter_get_css', 		'gordon_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'gordon_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'gordon_hovers_frontend_scripts', 1010 );
	function gordon_hovers_frontend_scripts() {
		if ( true )
			wp_enqueue_script( 'gordon-hovers', gordon_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( true )
			wp_enqueue_style( 'gordon-hovers',  gordon_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('gordon_hovers_merge_scripts')) {
	//Handler of the add_filter( 'gordon_filter_merge_scripts', 'gordon_hovers_merge_scripts' );
	function gordon_hovers_merge_scripts($list) {
...
if (!function_exists('gordon_hovers_merge_styles')) {
	//Handler of the add_filter( 'gordon_filter_merge_styles', 'gordon_hovers_merge_styles' );
	function gordon_hovers_merge_styles($list) {
...
if (!function_exists('gordon_hovers_localize_script')) {
	//Handler of the add_filter( 'gordon_filter_localize_script','gordon_hovers_localize_script' );
	function gordon_hovers_localize_script($arr) {
		$arr['button_hover'] = gordon_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('gordon_hovers_add_icons') ) {
	function gordon_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'gordon') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'gordon');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'gordon_hovers_get_css' ) ) {
	//Handler of the add_filter( 'gordon_filter_get_css', 'gordon_hovers_get_css', 10, 4 );
	function gordon_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_style_inverse.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link']} !important; }
...
.sc_button_hover_style_alter.sc_button_hover_slide_bottom  {	color: {$colors['inverse_link']} !important; }
